...
void init(struct GridGeom *G, struct FluidState *S)
{
  ...
  // Find the zone in which rCorad of Narayan condition resides
...
#pragma omp parallel for simd collapse(2)
  ZSLOOP(0, 0, -NG+1, N2+NG-1, -NG+1, N1+NG-1) {
    ...
    if (N3 > 1) {
      if (mad_type == SANE) {
        ...
        if ( r > rCorad || r < rBstart || fabs(q) > 1.e2 ) q = 0;
  ...
  ZLOOP {
    ...
    if ((r > rBstart && r < rCorad) || mad_type != NARAYAN) {
